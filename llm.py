...
class LLMEmailAssistant:
    ...
    def detect_intent (self, email_body: str, ) -> str:
        ...
        try:
            prompt = (
                ...
                "- information_request: Email asks something the assistant can answer based on general or internal knowledge.\n"
                "- web_search: Email asks for time-sensitive or publicly available information that typically requires checking official websites or online sources. This includes exam dates, application deadlines, registration portals, eligibility criteria, scholarship opportunities, latest announcements, rankings, or news.\n "
    ...
    def extract_search_query(self, email_body):
        prompt = f"""
        Based on the following email, generate a concise web search query that would help the assistant find information online to answer it.
    ...
    def generate_reply_with_web_results(self, email_body, search_results):
        prompt = f"""
        Use the following search results to generate a helpful and accurate reply to the user's email.
